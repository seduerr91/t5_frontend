...
# T5
model = T5ForConditionalGeneration.from_pretrained('./model_pai_con/').eval()
# model = T5ForConditionalGeneration.from_pretrained('seduerr/t5-pawraphrase').eval()
...
# works as: 
# fuse: crochet braids are known as a protective style. crochet braids are assist with hair growth.
# crochet braids are known as a protective style and assist with hair growth.

def _generate(sentence, num_sentences, max_len, top_p, early_stop):
    text = "contrast: " + sentence + " </s>"
...
if __name__ == '__main__':
    app.run(host='127.0.0.1', debug=True, port=8000, use_reloader=True)
